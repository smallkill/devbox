...
Engineering details: **corrects splits Yahoo left unadjusted** (Taiwan's ±10% daily limit makes an anomalous same-day ratio unambiguously a split → the artificial discontinuity is smoothed away, e.g. 0052's ~7:1 split in 2025-11), **fair comparison** over the common trading window across all picks, **URL-based sharing** (state encoded in the URL, optionally shortened via a self-hosted link service), and **security** (the proxy guards against SSRF and abuse; the token lives only in a Worker secret).
