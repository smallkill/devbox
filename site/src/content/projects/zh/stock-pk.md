...
落實工程細節:**修正 Yahoo 漏調的拆股**(台股 ±10% 漲跌幅限制使單日異常比值必為拆股 → 自動弭平假斷層,例如 0052 2025-11 的 7:1)、**公平比較**取所有標的的共同交易區間、**網址型分享**(狀態編進 URL、可一鍵用自架短網址服務縮短)、**安全**(代理端點防 SSRF 與濫用、token 僅存於 Worker secret)。
