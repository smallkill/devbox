...
`wrangler vectorize insert` 寫進 Vectorize index `resume`(1024 維,模型 `@cf/baai/bge-m3`)。
...
帳號 ID 已寫死在 `ingest.mjs`(`ACCOUNT_ID`)。

> Vectorize 的「寫入」不在本腳本範圍 —— 用 `wrangler vectorize insert`(走 wrangler OAuth)。
...
```bash
cd ask && npx wrangler vectorize insert resume --file .cache/vectors.ndjson
```
...
履歷內容變動時就要重跑,並重新 `vectorize insert`:
